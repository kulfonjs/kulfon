...
function compile(files) {
  ...
  const data = yaml.safeLoad(fs.readFileSync(path.join(currentDirectory, 'website/data.yml'), 'utf8'));
  ...
  for (let f of files) {
    ...
    switch (path.extname(f)) {
      ...
      case ".md":
        ...
        output = nunjucks.render(path.join('pages', path.parse(f).base), { data });
