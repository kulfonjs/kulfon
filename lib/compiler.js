...
const { unique, concat, merge, isEmpty } = require('./util');
...
function profile(func, prefix, allowedExtensions) {
  return async file => {
    ...
    print(`${startTime.toISOString().grey} - in ${prefix.blue} compiling ${file.yellow} `);

    const result = await func(file);

    let endTime = new Date();
    println(`${'done'.green} in ${endTime - startTime}ms`);
