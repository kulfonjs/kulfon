    ...
    case 'posts':
      return file => {
        perform(prefix, file, async () => {
          ...
          __posts[file] = { data: m.data, content: m.content };

          const post = md.render(m.content)

          let data = merge(__data, { post: __posts[file].data });
