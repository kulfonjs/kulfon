...
function compile(file) {
  ...
  switch (path.extname(file)) {
    ...
    case ".md":
      ...
      let m = matter.read(__current(file));
      ...
      output = nunjucks.renderString(__pages[file].content, {
        ...
        pages: () => {
          return Object.entries(__pages).map(([path, meta]) => Object.assign({}, meta.data, { path }));
        },
      });
...
function preprocess(files) {
  for (let file of files) {
    let subPath = path.join('pages', file);
    let fullPath = __current(subPath);
    let m = matter.read(fullPath);
    let { data, content } = m;

    __pages[subPath] = { data, content };
  }
