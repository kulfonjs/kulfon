...
function add({ asset }) {
  ...
  const packages = unique(config.includePaths
  ...
  if (! registry[name]) {
    console.log(`${'Error'.red}: ${name.yellow} is not yet supported. \nIf you know how to integrate that module manually, please contribute your solution to https://github.com/zaiste/kulfon/blob/master/registry.yml`)
    return;
  }
  ...
  fs.outputFileAsync(path.join(currentDirectory, 'config.yml'), yaml.safeDump(updatedConfig))
    .then(() => console.log(`Updating configuration... ${'done'.green}`))
