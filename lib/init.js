...
function init({ dir }) {
  ...
  Promise.all(Object.keys(paths).map(_ => initFile(_, paths[_])))
    .then(() => console.log('done'.green))
    .catch(() => console.log('done'.green));

  function initFile(filename, content) {
    const p = path.join(currentDirectory, dir, 'website', filename);
    return fs.outputFileAsync(p, content, { flag: 'wx' });
  }
}
